...
training_data <- wp[1:train_size, ]
test_data <- wp[(train_size + 1):n, ]
...
(results_df <- rbind(results_df, data.frame(Model = "Linear Regression", RMSE = lm_rmse, R2 = lm_r2))) # Add the results to summary data frame.
...
(results_df <- rbind(results_df, data.frame(Model = "Decision Tree", RMSE = tree_rmse_optimized, R2 = tree_r2_optimized))) # Add results to the summary data frame.
...
(results_df <- rbind(results_df, data.frame(Model = "Random Forest", RMSE = rf_rmse, R2 = rf_r2)))
...
(results_df <- rbind(results_df, data.frame(Model = "XGBoost", RMSE = xgb_rmse, R2 = xgb_r2)))
