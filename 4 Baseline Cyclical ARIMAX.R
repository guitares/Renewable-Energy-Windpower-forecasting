# 0.4 Cyclical + ARIMAX hybrid feature
library(pacman)
...
train$ARIMAX_hybrid <- residuals(arimax_model)
test$ARIMAX_hybrid <- 0

# 2.4 MLR----
# 2.4.1 MLR train
set.seed(123)
MLR_2.4 <- lm(active_power ~ ., data = train[, !names(train) %in% "Time"])
train_MLR_pred <- predict(MLR_2.4, newdata = train[, !names(train) %in% "Time"])
MLR_2.4.1_rmse <- RMSE(train_MLR_pred, train$active_power)
MLR_2.4.1_r2 <- R2(train_MLR_pred, train$active_power)
(results_df <- rbind(results_df, data.frame(Model = "2.4.1", RMSE = MLR_2.4.1_rmse, R2 = MLR_2.4.1_r2)))

# 2.4.2 MLR test
test_MLR_pred <- predict(MLR_2.4, newdata = test[, !names(test) %in% "Time"]) # Prediction
MLR_2.4.2_rmse <- RMSE(test_MLR_pred, test$active_power)
MLR_2.4.2_r2 <- R2(test_MLR_pred, test$active_power)
(results_df <- rbind(results_df, data.frame(Model = "2.4.2", RMSE = MLR_2.4.2_rmse, R2 = MLR_2.4.2_r2)))
...
set.seed(123)
tuned_tree_model_3.4 <- train(active_power ~ ., data = train[, !names(train) %in% "Time"], method = "rpart", trControl = train_control_ts, tuneGrid = tune_grid, control = control_params)
stopCluster(cl)
registerDoSEQ()

# 3.4.1 DT Train
train_predictions <- predict(tuned_tree_model_3.4, newdata = train[, !names(train) %in% "Time"])
DT_3.4.1_rmse <- RMSE(train_predictions, train$active_power)
DT_3.4.1_train_r2 <- R2(train_predictions, train$active_power)
results_df <- rbind(results_df, data.frame(Model = "3.4.1", RMSE = DT_3.4.1_rmse, R2 = DT_3.4.1_train_r2))

# 3.4.2 DT Test
test_predictions <- predict(tuned_tree_model_3.4, newdata = test[, !names(test) %in% "Time"])
DT_3.4.2_rmse <- RMSE(test_predictions, test$active_power)
DT_3.4.2_r2 <- R2(test_predictions, test$active_power)
(results_df <- rbind(results_df, data.frame(Model = "3.4.2", RMSE = DT_3.4.2_rmse, R2 = DT_3.4.2_r2)))
...
RF_4.4.1_rmse <- RMSE(rf_train_predictions, train$active_power)
RF_4.4.1_r2 <- R2(rf_train_predictions, train$active_power)
(results_df <- rbind(results_df, data.frame(Model = "4.4.1", RMSE = RF_4.4.1_rmse, R2 = RF_4.4.1_r2)))

# 4.4.2 RF test
rf_test_predictions <- predict(ranger_model, newdata = test[, !names(test) %in% "Time"])
RF_4.4.2_rmse <- RMSE(rf_test_predictions, test$active_power)
RF_4.4.2_r2 <- R2(rf_test_predictions, test$active_power)
(results_df <- rbind(results_df, data.frame(Model = "4.4.2", RMSE = RF_4.4.2_rmse, R2 = RF_4.4.2_r2)))
...
XGB_5.4.1_rmse <- RMSE(xgb_train_predictions, train$active_power)
XGB_5.4.1_r2 <- R2(xgb_train_predictions, train$active_power)
(results_df <- rbind(results_df, data.frame(Model = "5.4.1", RMSE = XGB_5.4.1_rmse, R2 = XGB_5.4.1_r2)))
...
XGB_5.4.2_rmse <- RMSE(xgb_test_predictions, test$active_power)
XGB_5.4.2_r2 <- R2(xgb_test_predictions, test$active_power)
(results_df <- rbind(results_df, data.frame(Model = "5.4.2", RMSE = XGB_5.4.2_rmse, R2 = XGB_5.4.2_r2)))
